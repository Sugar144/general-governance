...
2. Read, in order:
   ...
   - `framework/capabilities/work-packet-design/agent/work-packet-designer/role.md` — the full Designer role definition: inputs, the ordered Stage 0–8 workflow, cross-cutting judgment principles, stop/escalation conditions, and outputs. This is the authoritative source for how to do the work; this file only orients you toward it.
...
- **Perform the Stage 8 final adversarial check** before declaring the packet complete: ask what missing prerequisite would make an outcome unreachable, unvalidatable, or dishonestly complete. This does not substitute for an independently required Reviewer pass — see `contract.md` §5 for when one is mandatory.

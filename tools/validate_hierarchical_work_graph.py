#!/usr/bin/env python3
"""Deterministically validate General Governance HWG 1.0.0 structure.

This tool validates represented structure and exact identities only. It does not
infer semantic boundary preservation, authority, completion, execution readiness,
missing dependencies, or concurrent compatibility.
"""
...
BUNDLE_SCHEMA = ROOT / "contracts" / "hierarchical-work-graph-bundle.schema.json"
GRAPH_SCHEMA = ROOT / "contracts" / "work-graph.schema.json"
DISCOVERY_PATH = "configuration.capabilities.hierarchical_work_graph.bundle_path"
...
class CapabilityAbsent(RuntimeError):
    """The adopter configuration does not activate optional HWG."""
...
def validate_bundle(bundle_path: Path, *, source_root: Path | None = None) -> dict[str, Any]:
    ...
    for graph_id, graph in graphs.items():
        if graph_id == root_id:
            continue
        if graph["parent_binding"] is None:
    ...
    return {
        "status": "VALID_HIERARCHICAL_WORK_GRAPH",
...
def selected_bundle_path(configuration_path: Path, consumer_root: Path) -> Path:
    try:
        document = load_yaml(configuration_path)
    except (OSError, StrictYAMLError) as exc:
        fail("INVALID_ADOPTION", f"invalid adopter configuration {configuration_path}: {exc}")
    if not isinstance(document, dict) or not isinstance(document.get("configuration"), dict):
        fail("INVALID_ADOPTION", "adopter configuration must contain top-level configuration mapping")
    capabilities = document["configuration"].get("capabilities")
    if capabilities is None or not isinstance(capabilities, dict) or "hierarchical_work_graph" not in capabilities:
        raise CapabilityAbsent(DISCOVERY_PATH)
    hwg = capabilities["hierarchical_work_graph"]
    if not isinstance(hwg, dict):
        fail("INVALID_ADOPTION", "configuration.capabilities.hierarchical_work_graph must be a mapping")
    relative = hwg.get("bundle_path")
    if not isinstance(relative, str) or not relative.strip():
        fail("INVALID_ADOPTION", f"{DISCOVERY_PATH} must be a non-empty relative path")
    path = bounded_path(consumer_root, relative, DISCOVERY_PATH)
    if not path.is_file():
        fail("INVALID_ADOPTION", f"selected HWG bundle does not exist: {relative}")
    return path


def validate_selected_consumer_bundle(configuration_path: Path, consumer_root: Path) -> dict[str, Any] | None:
    try:
        bundle = selected_bundle_path(configuration_path.resolve(), consumer_root.resolve())
    except CapabilityAbsent:
        return None
    return validate_bundle(bundle, source_root=consumer_root.resolve())
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--consumer", type=Path, default=None)
    parser.add_argument("--configuration", type=Path, default=None)
    args = parser.parse_args(argv)
    try:
        if args.consumer is not None:
            if args.bundle is not None:
                fail("INVALID_CLI", "supply either a bundle or --consumer, not both")
            consumer = args.consumer.resolve()
            configuration = args.configuration
            if configuration is None:
                fail("INVALID_CLI", "--consumer mode requires --configuration")
            configuration = configuration if configuration.is_absolute() else consumer / configuration
            result = validate_selected_consumer_bundle(configuration, consumer)
            if result is None:
                print(json.dumps({"status": "CAPABILITY_ABSENT"}, sort_keys=True))
                return 0
        else:
            if args.bundle is None:
                fail("INVALID_CLI", "bundle path is required outside --consumer mode")

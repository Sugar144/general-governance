...
Status: `CANDIDATE_FOR_EMPIRICAL_ADOPTION`
...
HWG is intended to be owned by General Governance as reusable L2 work-governance semantics.

- General Governance owns the generic meaning of graph, node, dependency, parent/child expansion, completion eligibility, and structural validity.
...
Every `WorkGraph` MUST be acyclic. A dependency cycle is invalid.

### HWG-002 — Same-level dependency

A HWG dependency edge MUST reference a sibling node in the same graph. Arbitrary dependency edges directly between different hierarchy levels are invalid.

### HWG-003 — Exact parent

Every materialized child graph MUST bind to exactly one parent `(graph_id, node_id)` pair, and that parent node MUST bind back to the child graph identity.

### HWG-004 — Single current expansion

A parent node MUST NOT have more than one current `MATERIALIZED` child graph for the same expansion slot/profile. Alternative/historical expansions require an explicit superseded/history representation rather than ambiguous current children.

### HWG-005 — Boundary preservation

Expanding a node MUST refine the parent work/outcome boundary and MUST NOT silently redefine or broaden it. Semantic verification of this claim remains a review responsibility unless a future adopter-specific contract can prove it deterministically.
...
For the current software Factory profile, AEA D1/D2 is the designated JIT producer of `work packet → executable leaf DAG` expansion.
...
Every graph and node MUST carry sufficient adopter-owned source/provenance identity to explain where the represented work boundary came from. HWG does not infer source semantics from labels alone.
...
- HWG must reference that truth rather than become a competing product backlog;
...
The scheduler must not feed transient `READY/RUNNING/FAILED` state back into the normative HWG contract unless a future accepted lifecycle extension explicitly requires it.

## Candidate adoption path

This architecture is intentionally staged before a full GG release integration.

1. review the generic HWG semantics;
2. create one bounded Dopis projection using existing vertical-slice truth and current ordering WP/AEA evidence;
3. verify that the model does not force duplicate truth or stale leaf decomposition;
4. only then decide the minimum machine schema/validator and GG release/adoption surface required for normative activation;

...
The eight failure classes above are the Reviewer's mandatory minimum coverage: a review that fails to check for any one of them is incomplete. They do not exhaustively bound what the Reviewer may find. The Reviewer MAY also raise any other credible material finding — not confined to the eight enumerated classes — that demonstrates a violation of the WPDC normative contract, the adoption contract, an applicable authority boundary, historical-truth preservation, or the candidate's claimed semantic completeness. The Reviewer records each finding against the packet, cites the specific provision it violates (the WPDC section/invariant, or the adoption-contract/authority-boundary provision for a finding outside the eight classes), and returns a disposition recommendation.

This does not license the Reviewer to expand into unrelated generic code or product review: a finding must trace to one of the bases above — the WPDC normative contract, the adoption contract, an applicable authority boundary, historical-truth preservation, or the candidate's claimed semantic completeness — or it is out of scope for a finding and, if raised at all, is non-blocking commentary distinct from a finding.
...
- Independent Reviewer execution is mandatory whenever an existing applicable adopter/project governance, risk, assurance, authority, or review boundary requires independent review for the packet at hand. Where such existing governance instead permits proportional judgment — WPDC itself anchors this proportionality: "validation sufficiency requires domain judgment, that judgment remains semantic and may require independent review proportionate to the risk/materiality of the packet" (`contract.md` §9) — the agent layer uses that existing boundary rather than creating a WPDC-specific review policy of its own; a packet within that proportional-judgment space MAY be Designer-self-checked (§6 stage 8) without a separate Reviewer pass.
- This design fixes no numeric or universal materiality threshold of its own, consistent with WPDC's refusal to prescribe universal freshness/materiality intervals (WPDC §3.6). If a packet is materially dependent on independent review and no applicable governing boundary (adopter, project, risk, assurance, or authority) can determine whether independent review is required, the agent layer MUST stop and escalate rather than invent a threshold to resolve the ambiguity, mirroring the Designer's own stop/escalate duty (§6) for an unresolved required authority/source ambiguity.
- Designer self-review (§6 stage 8) never substitutes for an independently required Reviewer pass under the rule above; it is a compensating control available only within the proportional-judgment space, not an alternative to a governance-mandated independent review.
...
`tools/validate_consumer.py`'s `release_content_digest()` computes the release `content_sha256` identity by hashing every git-tracked repository path except `release-manifest.json` itself — not only paths named in `required_framework_surfaces`, which is an unrelated list. This design packet's three governance-only files are ordinary tracked content, so once integrated (merged to `main`) they affect that digest like any other tracked file.

The follow-on implementation packet will add further tracked content under `framework/capabilities/work-packet-design/agent/**` (and, if the fixture catalog in §10 is adopted as tracked content, under `tests/fixtures/work-packet-agent/**`), which affects the digest the same way. Consistent with the precedent set by Blocks 1–3 (`GG-WPDC-NORMATIVE-CAPABILITY-ADOPTION-001` block-result: "adding tracked framework content changes the release content digest while `release-manifest.json` still identifies immutable release candidate `0.1.0-rc.5`"), any release incorporating either this design packet's files or the follow-on implementation content therefore requires a separately governed new release identity rather than a modification of the current immutable `0.1.0-rc.6` candidate (`release-manifest.json`, `release_status: IMMUTABLE_RELEASE_CANDIDATE_PENDING_OWNER_DISPOSITION`). This design packet does not itself select, create, or modify that future release identity.
...
- how the agent layer, at implementation time, concretely identifies and binds the "applicable adopter/project governance, risk, assurance, authority, or review boundary" referenced by §8 for adopters that have not yet named one explicitly — §8 fixes the rule (an existing boundary governs; absent one, stop/escalate) but not the discovery mechanism, and this design deliberately does not invent a materiality threshold to fill that gap;
